...
# Load encoder (you should save and load it the same way you saved the model)
    ...
    if st.button('Make Prediction'):
        ...
        if result == 1:
            ...
            <div style="background-color:#d4edda;color: black; padding:10px; border-radius:5px; margin-bottom:10px;">
                <strong>✅ Prediction:</strong> Not Cancelled
            </div>
            """, unsafe_allow_html=True)
        else:
            st.markdown(f"""
            <div style="background-color:#f8d7da;color: black; padding:10px; border-radius:5px; margin-bottom:10px;">
                <strong>❌ Prediction:</strong> Cancelled
            </div>
            """, unsafe_allow_html=True)

def make_prediction(features):
    categorical_cols = ['type_of_meal_plan', 'required_car_parking_space', 'room_type_reserved', 'market_segment_type']
    ...
    for col in categorical_cols:
        try:
            ...
            input_data[col] = encoder.transform([encoder.classes_[0]])  # Use the first class as default
